...
// Utility Functions
const findImagesWithPropertyValue = (id, value, images = Immutable.List()) => {
    let imagesWithPropertyValue = Immutable.List()

    // Push any images on to the List have match the property value id and value.
    imagesWithPropertyValue = images.filter((image) =>
        image
            .get('variationProperties')
            .find(
                (variationPropety) =>
    ...
    // Filter out the variationProperties from the image.
    imagesWithPropertyValue = imagesWithPropertyValue.map((image) =>
        image.delete('variationProperties')
    )

    return imagesWithPropertyValue
}

const findOrderableVariationsWithPropertyValue = (id, value, variations = Immutable.List()) => {
    let variationsWithPropertyValue = Immutable.List()

    // Push any images on to the List have match the property value id and value.
    variationsWithPropertyValue = variations.filter(
        (variation) => variation.getIn(['values', id]) === value && variation.get('orderable')
    )

    return variationsWithPropertyValue
}
