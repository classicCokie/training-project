...
class ExampleProductDetails extends React.Component {
    ...
    getCarouselImages(product, variationValues = {}) {
        ...
        const fullImages = allImages
            .filter((image) => image.sizeType === 'large' || image.sizeType === 'default')
            .filter((image) => { // Only show the selected color if we have a color selected.
                if (!color) {
                ...
                const colorVariationProperty = variationProperties.find((variationProperty) => variationProperty.id === 'color') || {}
                const colorVariationPropertyValues = colorVariationProperty.values || []

                return colorVariationPropertyValues
                    .find((value) => value.value === color)
            })
    ...
    render() {
        ...
        return (
            <div className="t-example-product-details" itemScope itemType="http://schema.org/Product">
                ...
                <ListTile className="pw--instructional-block">
                    ...
                    <Button className="pw--primary" onClick={this.toggleShippingSheet.bind(this)} >
                        Modal Button
                    </Button>
                </ListTile>

                <ListTile className="pw--instructional-block">
                    <div className="u-margin-bottom-lg">Set up forms like a Join Mailing List section using this example:</div>
                ...
                <Sheet
                    ...
                    headerContent={
                        <HeaderBar>
                            ...
                            <HeaderBarActions>
                                <Button innerClassName="u-padding-0" icon="close" onClick={this.toggleShippingSheet.bind(this)} />
                            </HeaderBarActions>
                        </HeaderBar>
                    }
                >
                    <div className="t-example-product-details__shipping-delivery-modal-content">
                        Receive free Standard Shipping within Canada for purchases of $150+, excluding taxes, when signed into a Mobify.com account.
                    </div>
                </Sheet>
            </div>
        )
...
export default connect(mapStateToProps, mapDispatchToProps)(ExampleProductDetails)

// eslint-disable-next-line
const VariationPropertySwatchGroup = ({input: {value, onChange}, variationProperties = [], selectedVariationPropertyValue, label, error, id}) => {
    const selectedProperty = variationProperties.find(({value}) => value === selectedVariationPropertyValue)
    ...
    return (
        ...
        >
            {variationProperties.map(({name, value, swatches, orderable}) => {
                ...
                return (
                    <SwatchItem className="t-example-product-details__swatch" key={value} value={value} disabled={!orderable} analyticsName={id} analyticsContent={value}>
                        {swatches ?
                            (
                                <span className="t-example-product-details__swatch-background" style={swatchStyle}>
